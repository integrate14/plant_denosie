    ...
    for epoch in range(num_epochs):
        ...
        for noisy, clean in train_loader:
            ...
            if model_name == 'PointFilter':
                ...
                loss = model.get_loss(cleaned, clean)  # 正确：使用 cleaned (模型输出)
        ...
        with torch.no_grad():
            for noisy, clean in val_loader:
                ...
                if model_name == 'PointFilter':
                    ...
                    loss = model.get_loss(cleaned, clean)  # 使用 cleaned 而非 noisy
                elif model_name == 'StraightPCF':
                    cd = model.chamfer_distance(cleaned, clean)
                    ptp = torch.mean(torch.sqrt(torch.sum((cleaned - clean)**2, dim=-1) + 1e-8))
                    loss = cd + 0.1 * ptp
...
model_spcf = create_straightpcf_improved_model(num_points=2048, feature_dim=256, hidden_dim=128)
